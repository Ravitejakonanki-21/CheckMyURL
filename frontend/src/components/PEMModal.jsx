      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg max-w-4xl max-h-[90vh] w-full mx-4 flex flex-col border border-gray-400 dark:border-gray-600">
        ...
        <div className="flex-1 overflow-auto p-4">
          <div className="bg-black dark:bg-gray-800 rounded p-4 relative border border-gray-400 dark:border-gray-600">
            <pre className="text-sm text-[#00e5ff] font-mono overflow-x-auto whitespace-pre-wrap break-all">{pemData}</pre>
            <button onClick={() => navigator.clipboard?.writeText(pemData)} className="absolute top-2 right-2 text-[#00e5ff] hover:text-[#00ccf0] text-xs px-3 py-1 bg-black/50 rounded border border-[#00e5ff]/50" title="Copy PEM Certificate">
        ...
        <div className="p-4 border-t border-gray-300 dark:border-gray-700 flex justify-end">
          <button onClick={onClose} className="px-4 py-2 bg-[#00e5ff] text-[#0e0e0e] font-semibold rounded hover:bg-[#00ccf0] transition-colors duration-200">Close</button>
        </div>
